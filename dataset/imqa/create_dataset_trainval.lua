...
vgg_ind_train=torch.LongTensor(#train_ims);
for i=1,#train_ims do
	vgg_ind_train[i]=ims['list_of_images'][train_ims[i]];
end
vgg_ind_test=torch.LongTensor(#test_ims);
for i=1,#test_ims do
	vgg_ind_test[i]=ims['list_of_images'][test_ims[i]];
end
...
data_train['image_fvs']=ims['fvs']:index(1,vgg_ind_train);
...
data_test['image_fvs']=ims['fvs']:index(1,vgg_ind_test);
