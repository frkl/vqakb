cmd = torch.CmdLine();
cmd:text('Extract thought vectors (fc7) from a Caption-QA model');
cmd:text('Options')
...
cmd:option('-mode','caption','Which tv to extract? Can be "caption", "question" or "answer"');
...
function dataset:next_batch_caption(s,e)
	local timer = torch.Timer();
	local batch_size=e-s+1;
	local fv_cap=dataset['bow_cap'][{{s,e},{}}];
	return fv_cap:cuda(),batch_size;
end
...
function Forward(s,e)
	...
	if mode=='question' then
		...
		local scores=multimodal_net:forward({tv_q,torch.CudaTensor(e-s+1,nhcap):zero()});
	else
		local fv_im,batch_size=dataset:next_batch_caption(s,e);
	...
	elseif mode=='caption' then
		tvs=multimodal_net.modules[1].modules[10].output:double();
	end
	return tvs;
end
...
elseif mode=='caption' then
	ncaps=dataset['bow_cap']:size(1);
	tvs=torch.DoubleTensor(ncaps,nhcommon);
	for i=1,ncaps,batch_size do
		print(string.format('%d/%d, time %f',i,ncaps,timer:time().real));
		r=math.min(i+batch_size-1,ncaps);
		tvs[{{i,r},{}}]=Forward(i,r);
	end
